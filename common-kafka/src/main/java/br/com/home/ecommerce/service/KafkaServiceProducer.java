...
public class KafkaServiceProducer<T> implements Closeable{
	...
	public void send(String topico, String key, T value) {
		
		ProducerRecord<String, T> pedidoRecord = new ProducerRecord<>(topico, key, value);
		
		try {
			this.producer.send(pedidoRecord, sendCallback()).get();
